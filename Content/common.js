(function () {
    var currentWindowWidth = null,
        ...
        toastContainer = document.querySelector(".dbt-toast-container"),
    ...
    function setupTabControl(tabControl) {
        var container = ASPxClientUtils.GetParentByClassName(tabControl.GetMainElement(), "dbt-tab-container");
        var currentClassName = null;
        tabControl.ActiveTabChanging.AddHandler(function (s, e) {
            var className = "dbt-tab-state-" + s.GetActiveTabIndex() + "-" + e.tab.index;
    ...
    function updateSideNavState(sideNavEl) {
        sideNavEl.classList.toggle("dbt-side-nav-shown", !isSideNavCollapsed);
        sideNavEl.classList.toggle("dbt-side-nav-hidden", isSideNavCollapsed);
    }
    ...
    function setupHeaderToolbar(toolbar) {
        var sideNavEl = document.querySelector(".dbt-side-nav"),
            ...
            switch (e.item.name) {
                ...
                case "theme-dark":
                    if (e.item.GetChecked()) {
                        requestAnimationFrame(function () {
                            document.documentElement.classList.add("dbt-theme-switching");
                            setTimeout(function () {
                                requestAnimationFrame(function () {
                                    ...
                                    setTimeout(function () {
                                        requestAnimationFrame(function () {
                                            document.documentElement.classList.remove("dbt-theme-switching");
                                        });
                                    }, 200);
                                });
                            });
    ...
    ASPxClientControl.GetControlCollection().ControlsInitialized.AddHandler(function pageInitialized(controlCollection) {
        ...
        requestAnimationFrame(function () {
            document.documentElement.classList.remove("dbt-loading-process");
        });
    });
})();